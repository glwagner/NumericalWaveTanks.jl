...
epsilons = [10, 14, 18]
filename(ep) = "increasing_wind_ep$(ep)_k30_beta120_N384_384_256_L10_10_5_hi_freq_statistics.jld2"
dir = "data"
Δ_max = 1e-3
αsim = 0.6
...
for ep in epsilons
    statistics_filepath = joinpath(dir, statistics_filename(ep))
    ...
    lines!(ax_u, t_avg,   u_avg, linewidth=4, color=(:black,  αsim), label="Simulated mean(u)")

    lines!(ax_w, t_stats, w_max, linewidth=6, color=(:red, αsim), label="max(w)")
